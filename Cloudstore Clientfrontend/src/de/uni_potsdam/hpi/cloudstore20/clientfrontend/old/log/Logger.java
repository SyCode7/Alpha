package de.uni_potsdam.hpi.cloudstore20.clientfrontend.old.log;

import java.util.LinkedList;
import java.util.Queue;

import de.uni_potsdam.hpi.cloudstore20.clientfrontend.old.log.entry.BaseEntry;
import de.uni_potsdam.hpi.cloudstore20.clientfrontend.old.log.entry.LogEntry;
import de.uni_potsdam.hpi.cloudstore20.clientfrontend.old.log.threads.LogThread;
import de.uni_potsdam.hpi.cloudstore20.clientfrontend.old.log.threads.LogToFileThread;
import de.uni_potsdam.hpi.cloudstore20.clientfrontend.old.log.threads.LogToSysOutThread;
...
public class Logger {
	...
	public enum LOGABLE_METHODS {
		t_erasure, t_native, t_parallel, t_opt_erasure, t_unknown
	};

	public static enum LOGABLE_LOGS {
		...
		/**
		 * Thread task - uploading start-timestamp (one packets to one
		 * providers)
		 */
		...
		public static boolean includes(String log) {
			...
			for (LOGABLE_LOGS l : LOGABLE_LOGS.values()) {
	 ...
	 * {@link Logger#setUp(long, String, LoggerType)} to pass needed settings
	 * for the Logger.
	 * 
	 * */
	...
	/**
	 * Call this method to setup the Logger before calling
	 * {@link Logger#getInstance()}. Otherwise {@link Logger#getInstance()} will
	 * return <code>null</code>.
	 * 
	 * */
	public static void setUp(long timeOut, String locationOrDbName,
			LoggerType type) {

		if (timeOut < 0 || locationOrDbName == null
				|| locationOrDbName.length() == 0) {
	...
	/**
	 * Resets all settings. See also
	 * {@link Logger#setUp(long, String, LoggerType)}
	 */
	...
	private LogThread newThread() {
		...
		switch (Logger.TYPE) {
		case File:
			return new LogToFileThread(Logger.getTimeOut(),
					Logger.getLocation());
		case SysOut:
		default:
	...
	// ist zum zwischen speichern der Logs gedacht, die nicht gleich geschrieben
	// werden sollen
	public LogEntry getLogEntry(String id, LOGABLE_LOGS log, String attribute) {
	...
	private LogEntry getProviderLogEntry(String id, LOGABLE_LOGS log,
			long timeStamp, String providerName, String location, long fileSize) {
	...
	public void addRawLog(String id, long timestamp, LOGABLE_LOGS log,
			String attribute) {
	...
	// ist zum schreiben der log gedacht, die nicht gleich geschrieben werden
	// sollen
	...
	/**
	 * adding Task-Logs (<code>LOG.REQUEST_...</code> and
	 * <code>LOG.RESPONSE_...</code>)
	 * */
	public void addLog(String id, LOGABLE_LOGS log, String attribute, int k,
			int m, LOGABLE_METHODS method, String action, String fileName) {
	...
	/**
	 * Adding Test-Logs (<code>LOG.START_TEST</code> and
	 * <code>LOG.END_TEST</code>)
	 * */
	public void addLog(String id, LOGABLE_LOGS log, String attribute,
			String testClass) {
	...
	public void addLog(String id, LOGABLE_LOGS log) {
	...
	public void addLog(String id, ProviderReturnValueBoolean value,
			String providerName, String location, long fileSize) {

		this.addThreadSafe(getProviderLogEntry(id, LOGABLE_LOGS.START_DOWNLOAD,
				value.getValue(LOGABLE_LOGS.START_DOWNLOAD), providerName,
				location, fileSize));
		this.addThreadSafe(getProviderLogEntry(id, LOGABLE_LOGS.END_DOWNLOAD,
				value.getValue(LOGABLE_LOGS.END_DOWNLOAD), providerName,
				location, fileSize));
	...
	public void addLog(String id, ProviderReturnValueString value,
			String providerName, String location, long fileSize) {

		this.addThreadSafe(getProviderLogEntry(id, LOGABLE_LOGS.START_UPLOAD,
				value.getValue(LOGABLE_LOGS.START_UPLOAD), providerName,
				location, fileSize));
		this.addThreadSafe(getProviderLogEntry(id, LOGABLE_LOGS.END_UPLOAD,
				value.getValue(LOGABLE_LOGS.END_UPLOAD), providerName,
				location, fileSize));
	...
	public void addError(String id, String message, String providerName,
			String location, long size) {

		BaseEntry b = this.getLogEntry(id, LOGABLE_LOGS.ERROR, message);
