...
import de.uni_potsdam.hpi.cloudstore.log.*;
...
public class LogToFileThread extends LogThread {
	...
	private void createMetaFile() throws IOException {
		...
		Logger.LOG[] logs = Logger.LOG.values();
		...
		try {
			...
			for (BaseEntry entry : entryList) {
				...
				if (logentry != null) {
					...
					if (!Logger.LOG.includes(log)) {
						this.addNewLogToMetaFile(logentry.split(LogEntry.DELIMITER)[2]);
					}
				}
			}
	...
	boolean checkSettings() {
		...
		if (this.sleepingTime > 0 && this.location != null && this.location.isDirectory()) {
			return true;
		}
		return false;
	}
